# Script to simulate GitFlow branching model
$demoDir = Join-Path $PSScriptRoot "branch-demo"
...
# 1. Main branch
git checkout -b main
"Initial production code" | Set-Content app.txt
git add app.txt
git commit -m "Initial commit on main"

# 2. Develop branch
git checkout -b develop
"Integration code" | Add-Content app.txt
git commit -am "Setup develop branch"

# 3. Feature branch
git checkout -b feature/user-auth develop
"User authentication logic" | Set-Content auth.txt
git add auth.txt
...
"Version 1.0.0" | Set-Content version.txt
git add version.txt
...
"Hotfix: Fix critical bug" | Add-Content app.txt
...
Write-Host "`nGit Branching Graph Output:"
